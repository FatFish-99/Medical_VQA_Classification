...
a maximum of 128 tokens. After routing, the binary subset contains 940 samples (750 train / 190 val),
nearly balanced at 49.5\% / 50.5\% yes/no. The open-ended subset contains the
remaining 3,853 samples (3,084 train / 769 val), comprising all non-binary
answers and drawn primarily from ROCO.
...
queried via the OpenAI API zero-shot, with each image base64-encoded and
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    BLIP (zero-shot)         & 0.0000 & 0.0518 & 0.0427 & 0.6539 & 0.7019 & 0.6750 \\
    BLIP (fine-tuned)        & 0.0000 & 0.0953 & 0.0873 & 0.6812 & 0.7025 & 0.6905 \\
    BLIP-2 (zero-shot)       & 0.0039 & 0.1409 & 0.1251 & 0.7559 & 0.7304 & 0.7421 \\
    InstructBLIP (zero-shot) & 0.0058 & \textbf{0.1517} & \textbf{0.1345} & 0.7609 & 0.7353 & 0.7467 \\
    GPT-5 Mini (zero-shot)   & \textbf{0.0088} & 0.1215 & 0.0943 & 0.7107 & \textbf{0.8021} & \textbf{0.7528} \\
    \bottomrule
  \end{tabular}
\end{table}

BERTScore F1 improves consistently across all five models (0.675 $\to$ 0.753),
even as ROUGE rankings are less stable. Fine-tuning BLIP's language decoder
nearly doubles ROUGE-1 (0.052 to 0.095), confirming domain adaptation helps at
small scale. BLIP-2 zero-shot surpasses fine-tuned BLIP on all metrics,
showing that a larger language backbone (OPT-2.7B) provides more value than
task-specific training with limited data.

The central finding is a rank inversion between ROUGE and BERTScore for GPT-5
Mini. InstructBLIP achieves the highest ROUGE scores (ROUGE-1: 0.152), while
GPT-5 Mini leads on BERTScore F1 (0.7528) and Recall (0.8021 vs.\ 0.735). This
occurs because GPT-5 Mini generates verbose structured clinical reports while
ROCO references are single-sentence labels---ROUGE penalises every unmatched
token, whereas BERTScore captures semantic equivalence in embedding space.
GPT-5 Mini's lower Precision (0.711) reflects additional clinical detail beyond
the reference, not inaccuracy. This result highlights a methodological
limitation: ROUGE underestimates model quality when generated text is richer
than ground-truth annotations, making BERTScore the more appropriate primary
metric for evaluating frontier VLMs on short-caption datasets.
...
(17.1\,GB VRAM) locally; BLIP fine-tuning completed in under 2 hours and each
zero-shot model took 30--45 minutes. GPT-5 Mini inference over 769 samples took
...
zero-shot, while InstructBLIP leads on ROUGE, revealing that metric choice
