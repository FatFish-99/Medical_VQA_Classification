...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/sample_images.png}
...
The preprocessing pipeline prepares image and text inputs for the model. Images
are resized to $224 \times 224$ pixels, converted to three-channel RGB, and
normalized using ImageNet statistics (mean $[0.485, 0.456, 0.406]$, std
$[0.229, 0.224, 0.225]$) to match the pretrained CNN/ResNet encoder's expected
format. Questions are tokenized with a maximum length of 128 tokens, with
truncation and padding applied as needed. 
We used the dataset's official
train/validation splits. After filtering for binary yes/no examples, the
training set contained 750 samples (379 no, 371 yes) and the validation set
contained 190 samples (94 no, 96 yes), yielding a near-perfect 50/50 class
balance in both splits. The open-ended subset contains the remaining 3,853
samples (3,084 train / 769 val), drawn primarily from ROCO.
...
\begin{figure}[h]
  ...
  \caption{ROC curves for all binary VQA models. The fine-tuned BioBERT
           fusion (AUC\,=\,0.8281) consistently outperforms the vision-only
           ResNet-18 baseline (AUC\,=\,0.7740) and text-only baselines.}
  \label{fig:roc}
\end{figure}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.4\linewidth]{figures/qualitative_sample.png}
  \caption{Sample CT image from the validation set used in the qualitative analysis.}
